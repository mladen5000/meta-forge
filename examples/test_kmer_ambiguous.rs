...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match extractor.extract_minimizers(problem_sequence) {
        Ok(minimizers) => {
            println!("✅ Skip strategy succeeded! Found {} minimizers (skipped problematic k-mers)", minimizers.len());
        }
        Err(e) => {
    ...
    match extractor.extract_minimizers(problem_sequence) {
        Ok(minimizers) => {
            println!("✅ Allow strategy succeeded! Found {} minimizers", minimizers.len());
            for (i, min) in minimizers.iter().take(3).enumerate() {
                println!("   Minimizer {}: {} (pos: {})", i+1, min.kmer.sequence, min.position);
            }
        }
        Err(e) => {
    ...
    match extractor.extract_minimizers(problem_sequence) {
        Ok(minimizers) => {
            println!("✅ Replace strategy succeeded! Found {} minimizers", minimizers.len());
            for (i, min) in minimizers.iter().take(3).enumerate() {
                println!("   Minimizer {}: {} (pos: {})", i+1, min.kmer.sequence, min.position);
            }
        }
    ...
    match extractor.extract_minimizers(problem_sequence) {
        Ok(minimizers) => {
            println!("✅ Context replace strategy succeeded! Found {} minimizers", minimizers.len());
            for (i, min) in minimizers.iter().take(3).enumerate() {
                println!("   Minimizer {}: {} (pos: {})", i+1, min.kmer.sequence, min.position);
            }
        }
    ...
    match extractor_default.extract_minimizers(problem_sequence) {
        Ok(minimizers) => {
            println!("✅ Default behavior succeeded! Found {} minimizers", minimizers.len());
        }
        Err(e) => {
