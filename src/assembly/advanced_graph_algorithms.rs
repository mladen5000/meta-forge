...
impl AdvancedDeBruijnGraph {
    ...
    fn analyze_component(&self, component: &[u64]) -> StronglyConnectedComponent {
        ...
        let total_coverage: u32 = component.iter().map(|&id| self.nodes[&id].coverage).sum();
    ...
    pub fn generate_contigs(&mut self) -> Result<Vec<GraphContig>> {
        ...
        for component in components {
            match component.component_type {
                ...
                ComponentType::Complex => {
                    let complex_contigs =
                        self.generate_complex_contigs(&component, &mut contig_id)?;
    ...
    fn remove_node_and_edges(&mut self, node_id: u64) -> Result<()> {
        ...
        if let Some(removed_node) = self.nodes.remove(&node_id) {
            self.edge_count = self
                .edge_count
                .saturating_sub(removed_node.outgoing.len() + removed_node.incoming.len());
        }
...
mod tests {
    ...
    fn test_contig_generation() {
        ...
        for contig in &contigs {
            println!(
                "Contig {}: {} nodes, length {}, coverage {:.1}, confidence {:.2}",
                contig.id,
                contig.node_path.len(),
                contig.length,
                contig.coverage,
                contig.confidence
            );
