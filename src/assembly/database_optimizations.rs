...
impl ConnectionPool {
    fn new(database_path: &str, max_size: usize) -> Result<Self> {
        ...
        for _ in 0..max_size {
            let mut conn =
                Connection::open(database_path).context("Failed to create database connection")?;
...
impl GenomicDatabase {
    ...
    pub fn batch_insert_kmers(&self, kmers: &[(String, Vec<u8>, u32)]) -> Result<usize> {
        ...
        if kmers.len() > 1000 {
            println!(
                "💾 Database: Inserting {} k-mers in batches of {}",
                kmers.len(),
        ...
        if kmers.len() > 1000 {
            println!(
                "📊 Database Performance: {} k-mers/sec | Total time: {}ms | Memory efficient: {}",
                rate,
                elapsed_ms,
...
mod tests {
    ...
    fn test_database_basic_operations() -> Result<()> {
        ...
        let db = GenomicDatabase::new(db_path.to_str().unwrap(), DatabaseConfig::default())?;
    ...
    fn test_caching_performance() -> Result<()> {
        ...
        // Insert test data
        let kmers = (0..1000)
            .map(|i| (format!("KMER{:04}", i), vec![i as u8; 4], (i % 100) as u32))
            .collect::<Vec<_>>();
