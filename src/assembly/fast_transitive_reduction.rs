...
impl FastTransitiveReducer {
    ...
    pub fn reduce_graph(&self, graph: &mut Graph<u32, (), Directed>) -> Result<usize> {
        ...
        if self.verbose {
            println!("   ✅ Topological sort: {:.3}ms", topo_start.elapsed().as_millis());
        }
        ...
        if self.verbose {
            println!("   ✅ Reduction analysis: {:.3}ms", reduction_start.elapsed().as_millis());
            println!("   📊 Removed {} redundant edges ({:.1}% reduction)", 
                     edges_removed, (edges_removed as f64 / edge_count.max(1) as f64) * 100.0);
            println!("   ⏱️  Total time: {:.3}ms (vs ~{}s with Floyd-Warshall)", 
                     start_time.elapsed().as_millis(),
                     (node_count.pow(3) / 1_000_000).max(1)); // Rough Floyd-Warshall estimate
        }
    ...
    fn reduce_with_bitsets(&self, graph: &mut Graph<u32, (), Directed>, topo_order: &[NodeIndex]) -> Result<usize> {
        let node_count = graph.node_count();
    ...
    fn reduce_with_hashsets(&self, graph: &mut Graph<u32, (), Directed>, topo_order: &[NodeIndex]) -> Result<usize> {
    ...
    fn has_path_bfs(&self, graph: &Graph<u32, (), Directed>, start: NodeIndex, end: NodeIndex) -> bool {
        if start == end {
            return true;
        }
...
mod tests {
    ...
    fn test_performance_comparison() {
        ...
        println!("Testing with {} nodes, {} edges", graph.node_count(), graph.edge_count());
