...
use crate::assembly::optimized_structures::{
    CompactKmer, MemoryBenchmark, StreamingKmerProcessor, UnifiedAssemblyGraph,
};
use crate::core::data_structures::{CanonicalKmer, GraphEdge, GraphFragment, GraphNode};
use anyhow::Result;
use rayon::prelude::*;
...
impl AssemblyMemoryBenchmark {
    ...
    fn benchmark_kmer_representation(&mut self) -> Result<()> {
        ...
        let string_kmers: Vec<CanonicalKmer> = self
            .test_data
            .medium_kmers
            .par_iter()
        ...
        let baseline_memory = string_kmers.len() * (std::mem::size_of::<CanonicalKmer>() + 32); // Estimated string overhead
        benchmark.record_baseline(baseline_memory);

        // Optimized: Bit-packed k-mers
        let optimized_start = Instant::now();
        let compact_kmers: Vec<CompactKmer> = self
            .test_data
            .medium_kmers
            .par_iter()
        ...
        let optimized_memory = compact_kmers
            .iter()
    ...
    fn measure_baseline_transitive_memory(&self, kmers: &[String]) -> Result<usize> {
        ...
        let graph_size =
            n * std::mem::size_of::<GraphNode>() + (n * n / 10) * std::mem::size_of::<GraphEdge>(); // Assume 10% connectivity
    ...
    fn print_comprehensive_results(&self) {
        ...
        println!(
            "Total Baseline Memory:   {:.2} MB",
            total_baseline as f64 / (1024.0 * 1024.0)
        );
        println!(
