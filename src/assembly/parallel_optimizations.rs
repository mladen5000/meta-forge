...
impl ParallelProcessor {
    ...
    pub fn process_kmers_parallel<T, F, R>(&self, data: &[T], processor: F) -> Vec<R>
    where
        T: Send + Sync,
        F: Fn(&T) -> R + Send + Sync,
        R: Send,
    {
        ...
        let results: Vec<R> = data
            ...
            .map(|chunk| chunk.iter().map(&processor).collect::<Vec<_>>())
            .flatten()
            .collect();
    ...
    fn update_metrics(&self, processed_items: usize, elapsed: std::time::Duration) {
        self.metrics
            .total_processed
            .fetch_add(processed_items, Ordering::Relaxed);
        self.metrics
            .total_time_us
    ...
    fn auto_tune_chunk_size(&self, current_throughput: f64) {
        ...
        } else if chunks_processed as f64 / self.num_threads as f64
            > target_chunks_per_thread as f64 * 2.0
        {
    ...
    fn merge_hashmaps_parallel(
        ...
        maps.into_par_iter().reduce(AHashMap::new, |mut acc, map| {
...
mod tests {
    ...
    fn test_transitive_reduction_performance() {
        ...
        let removed = processor
            .transitive_reduction_parallel(&mut adjacency)
            .unwrap();
        let elapsed = start.elapsed();

        println!(
            "Transitive reduction: removed {} edges in {:?}",
            removed, elapsed
        );
