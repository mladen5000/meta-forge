...
use anyhow::Result;
use meta_forge::assembly::fast_kmer_extraction::{hash_to_kmer, FastKmerExtractor};
...
fn main() -> Result<()> {
    ...
    for (num_reads, read_length) in test_cases {
        ...
        if improvement_factor > 1.0 {
            println!(
                "   📈 Performance improvement: {:.1}x faster than baseline!",
                improvement_factor
            );
        }
