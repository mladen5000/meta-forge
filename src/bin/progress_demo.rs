...
fn generate_realistic_sequences(num_reads: usize, read_length: usize) -> Vec<Vec<u8>> {
    ...
    println!("🧬 Generating {} realistic genomic sequences of {}bp each...", num_reads, read_length);
    ...
    for i in 0..num_reads {
        ...
        if i % (num_reads / 10) == 0 && i > 0 {
            println!("  📊 Generated {}/{} sequences ({:.1}%)", i, num_reads, (i as f64 / num_reads as f64) * 100.0);
        }
    }
...
fn main() -> Result<()> {
    ...
    let (kmer_results, progress_updates) = extractor.extract_kmers_from_sequences_with_progress(&sequences)?;
    
    let duration = start.elapsed();
    
    println!("📊 Smooth Progress Updates Captured:");
    for (i, (current_reads, total_reads, current_kmers, reads_per_sec, _timestamp)) in progress_updates.iter().enumerate() {
        let progress_pct = (*current_reads as f64 / *total_reads as f64) * 100.0;
        println!("  Update {}: {}/{} reads ({:.1}%) → {} k-mers | {:.0} reads/sec", 
                 i + 1, current_reads, total_reads, progress_pct, current_kmers, reads_per_sec);
    }
    
    println!();
    println!("🎉 Ultra-Fast K-mer Extraction Results:");
    println!("  ⏱️  Total time: {:.3}s", duration.as_secs_f64());
    println!("  🚀 Processing rate: {:.0} reads/second", num_reads as f64 / duration.as_secs_f64());
    println!("  🧮 Unique k-mers found: {}", kmer_results.len());
    println!("  📊 Total k-mer instances: {}", kmer_results.iter().map(|(_, count)| count).sum::<u32>());
