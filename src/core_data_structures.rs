...
impl CanonicalKmer {
    pub fn new(kmer: &str) -> Result<Self> {
        if !kmer.chars().all(|c| matches!(c, 'A' | 'C' | 'G' | 'T' | 'a' | 'c' | 'g' | 't')) {
    ...
    fn reverse_complement(seq: &str) -> Result<String> {
        ...
        String::from_utf8(rc_bytes).map_err(|e| anyhow!("Invalid UTF-8 in reverse complement: {}", e))
    }
...
impl GraphFragment {
    ...
    pub fn merge_with(&mut self, other: GraphFragment) -> Result<()> {
        ...
        for (hash, other_node) in other.nodes {
            if let Some(existing_node) = self.nodes.get_mut(&hash) {
                ...
                existing_node.read_positions.extend(other_node.read_positions);
        ...
        for other_edge in other.edges {
            ...
            if let Some(existing_edge) = self.edges.iter_mut().find(|e| 
                e.from_hash == other_edge.from_hash && e.to_hash == other_edge.to_hash
            ) {
                existing_edge.weight += other_edge.weight;
                existing_edge.supporting_reads.extend(other_edge.supporting_reads);
    ...
    fn update_coverage_stats(&mut self) {
        ...
        if !coverages.is_empty() {
            self.coverage_stats.mean_coverage = coverages.iter().sum::<u32>() as f64 / coverages.len() as f64;
    ...
    pub fn get_adjacency_list(&self) -> AHashMap<u64, Vec<u64>> {
        ...
        for edge in &self.edges {
            adjacency.entry(edge.from_hash)
    ...
    fn trace_path(&self, start: u64, adjacency: &AHashMap<u64, Vec<u64>>, max_length: usize) -> Option<Vec<u64>> {
    ...
    fn detect_reconvergence(&self, paths: &[Vec<u64>], start_node: u64) -> Option<BubbleStructure> {
        ...
        let end_nodes: HashSet<u64> = paths.iter()
        ...
        if end_nodes.len() == 1 {
            ...
            Some(BubbleStructure {
                ...
                bubble_type: if paths.len() == 2 { BubbleType::Simple } else { BubbleType::Complex },
            })
        } else {
            None
        }
...
impl AssemblyChunk {
    ...
    fn add_or_update_node(&mut self, kmer: &CanonicalKmer, read_id: usize, position: usize) -> Result<()> {
