...
impl MetagenomicsDatabase {
    ...
    pub fn store_assembly_results(
        ...
        self.caches
            .assembly_cache
            .insert(sample_name.to_string(), record);
    ...
    pub fn get_taxonomy_name(&self, taxonomy_id: u32) -> Result<Option<String>> {
        ...
        let name: Option<String> = stmt.query_row([taxonomy_id], |row| row.get(0)).optional()?;

        // Cache result
        if let Some(ref name_str) = name {
            self.caches
                .taxonomy_cache
                .insert(taxonomy_id, name_str.clone());
        }
    ...
    pub fn find_similar_sequences(
        ...
        for kmer in &query_kmers {
            ...
            for row_result in rows {
                ...
                let entry = sequence_scores
                    .entry(sequence_id)
                    .or_insert((0, taxonomy_id, length));
    ...
    pub fn get_assembly_stats(&self, sample_name: &str) -> Result<Option<AssemblyRecord>> {
        ...
        if let Some(ref rec) = record {
            self.caches
                .assembly_cache
                .insert(sample_name.to_string(), rec.clone());
        }
    ...
    pub fn build_kmer_index(&self, k_size: usize) -> Result<()> {
        ...
        let sequences = {
            let mut seq_stmt =
                tx.prepare("SELECT id, sequence_data FROM sequences WHERE length >= ?1")?;
    ...
    pub fn get_database_stats(&self) -> Result<DatabaseStats> {
        ...
        stats.taxonomy_count =
            conn.query_row("SELECT COUNT(*) FROM taxonomy", [], |row| row.get(0))?;
        stats.sequences_count =
            conn.query_row("SELECT COUNT(*) FROM sequences", [], |row| row.get(0))?;
        stats.features_count =
            conn.query_row("SELECT COUNT(*) FROM sequence_features", [], |row| {
                row.get(0)
            })?;
        stats.assemblies_count =
            conn.query_row("SELECT COUNT(*) FROM assemblies", [], |row| row.get(0))?;
        stats.contigs_count =
            conn.query_row("SELECT COUNT(*) FROM contigs", [], |row| row.get(0))?;
        stats.annotations_count =
            conn.query_row("SELECT COUNT(*) FROM annotations", [], |row| row.get(0))?;
        stats.kmer_index_count =
            conn.query_row("SELECT COUNT(*) FROM kmer_index", [], |row| row.get(0))?;
...
impl DatabaseMigrator {
    ...
    pub fn import_sequences_from_fasta<P: AsRef<Path> + std::fmt::Debug>(
        &self,
        fasta_file: P,
        source: &str,
    ) -> Result<()> {
