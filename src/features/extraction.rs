...
impl AdvancedFeatureExtractor {
    ...
    pub fn extract_sequence_features(&self, sequence: &str) -> Result<FeatureVector> {
        ...
        let combined_features =
            self.combine_features(&sequence_features, &Array1::zeros(0), &kmer_features_array)?;
    ...
    pub fn extract_composition_features(&self, sequence: &str) -> Result<Vec<f64>> {
        ...
        for window in sequence.as_bytes().windows(2) {
            if let Ok(dinuc) = std::str::from_utf8(window) {
                if let Some(pos) = dinucleotides
                    .iter()
                    .position(|&x| x.eq_ignore_ascii_case(dinuc))
                {
                    dinuc_counts[pos] += 1;
                }
            }
            ...
            for amino_acid in &[
                ...
                if aa_total > 0 {
                    ...
                    for codon in aa_codons {
                        let observed_freq = codon_counts.get(codon.as_str()).copied().unwrap_or(0)
                            as f64
                            / aa_total as f64;
                        let rscu = if expected_freq > 0.0 {
    ...
    fn extract_pattern_features(&self, sequence: &str) -> Result<Vec<f64>> {
        ...
        features.push(
            self.pattern_recognizers
                .detect_low_complexity_regions(sequence),
        );
    ...
    fn extract_local_topology_features(
        &self,
        node: &GraphNode,
        graph: &AssemblyGraph,
    ) -> Result<Vec<f64>> {
    ...
    fn combine_features(
        &self,
        seq_features: &Array1<f64>,
        graph_features: &Array1<f64>,
        kmer_features: &Array1<f64>,
    ) -> Result<Array1<f64>> {
...
impl KmerSignatures {
    fn new(kmer_sizes: &[usize]) -> Result<Self> {
        ...
        for _ in 0..16 {
            // 16 hash functions for signatures
        ...
        for period in 2..=4 {
            ...
            for i in 0..sequence.len().saturating_sub(period) {
                if let (Some(a), Some(b)) =
                    (sequence.chars().nth(i), sequence.chars().nth(i + period))
                {
                    if a == b {
                        period_score += 1.0;
                    }
