...
impl<'a> ProgressWidget<'a> {
    ...
    pub fn render(&self, f: &mut Frame, area: Rect) {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(status_text, Style::default().fg(match self.progress.status {
        ...
        if let Some(rate) = self.progress.rate {
            ...
            if let Some(eta) = self.progress.eta {
                rate_line.extend(vec![
                    Span::raw(" | ETA: "),
                    Span::raw(format_duration(eta)),
                ]);
            }
...
impl<'a> MultiProgressWidget<'a> {
    ...
    pub fn render(&self, f: &mut Frame, area: Rect) {
        ...
        let items: Vec<ListItem> = self.operations
            .iter()
            .enumerate()
            .map(|(i, (name, progress))| {
