...
fn test_assembly_chunk_finalize() {
    ...
    println!("📖 Input read: '{}' ({}bp)", test_read.corrected, test_read.corrected.len());
            ...
            match CanonicalKmer::new(kmer_str) {
                Ok(canonical) => {
                    println!("  K-mer {}: '{}' -> canonical '{}' (hash={})", 
                            i+1, kmer_str, canonical.sequence, canonical.hash);
                }
                Err(e) => {
    ...
    println!("  GraphFragment nodes: {}", chunk.graph_fragment.nodes.len());
    println!("  GraphFragment edges: {}", chunk.graph_fragment.edges.len());
    ...
    if chunk.graph_fragment.nodes.is_empty() {
        ...
        for (i, read) in chunk.reads.iter().enumerate() {
            println!("  Read {}: '{}' ({}bp)", i, read.corrected, read.corrected.len());
        }
        
        panic!("AssemblyChunk.finalize() is not working correctly!");
    } else {
        println!("✅ AssemblyChunk.finalize() created {} nodes and {} edges", 
