...
fn test_parallel_contig_generator_basic() {
    ...
    println!("📊 Graph stats: {} nodes, {} edges, {}KB memory, {:.1}% cache hit", 
             nodes, edges, memory/1024, cache_rate*100.0);
    ...
    match ParallelContigGenerator::generate_contigs_parallel(&graph) {
        Ok(contigs) => {
            ...
            for (i, contig) in contigs.iter().enumerate() {
                println!("  Contig {}: {}bp, coverage={:.1}x, sequence='{}'", 
            ...
            if contigs.is_empty() {
                println!("🚨 BUG DETECTED: Zero contigs generated from {} connected nodes", nodes);
...
fn test_parallel_contig_generator_unconnected() {
    ...
    match ParallelContigGenerator::generate_contigs_parallel(&graph) {
        Ok(contigs) => {
            println!("✅ Generated {} contigs from {} unconnected nodes", contigs.len(), nodes);
            
            // Each isolated node should become its own contig
            assert_eq!(contigs.len(), nodes, 
                      "Unconnected nodes should produce {} contigs, got {}", nodes, contigs.len());
            
            for (i, contig) in contigs.iter().enumerate() {
                println!("  Isolated contig {}: {}bp, sequence='{}'", 
...
fn test_kmer_extraction_basics() {
    ...
    for (i, expected) in expected_kmers.iter().enumerate() {
        match CanonicalKmer::new(expected) {
            Ok(canonical) => {
                println!("  K-mer {}: '{}' -> canonical '{}' (hash={})", 
                        i+1, expected, canonical.sequence, canonical.hash);
...
fn test_assembly_chunk_processing() {
    ...
    println!("📊 GraphFragment after finalization: {} nodes, {} edges", nodes, edges);
    ...
    for (hash, node) in &chunk.graph_fragment.nodes {
        println!("  Node {}: coverage={}, k-mer='{}'", hash, node.coverage, node.kmer.sequence);
    }
    
    // Print edge information
    for (i, edge) in chunk.graph_fragment.edges.iter().enumerate() {
        println!("  Edge {}: {} -> {} (weight={})", i+1, edge.from_hash, edge.to_hash, edge.weight);
    }
...
fn test_full_pipeline_simple() {
    ...
    for (i, read) in reads.iter().enumerate() {
        ...
        chunk.add_read(read.clone()).expect("Read addition should succeed");
    }
    ...
    let nodes = chunk.graph_fragment.nodes.len();
    let edges = chunk.graph_fragment.edges.len();
    println!("📊 After chunk processing: {} nodes, {} edges", nodes, edges);
    
    if nodes == 0 {
        panic!("🚨 PIPELINE FAILURE: No nodes created from {} reads", reads.len());
    }
    ...
    println!("📊 CacheOptimizedGraph: {} nodes, {} edges", final_nodes, final_edges);
    
    // Step 3: Generate contigs
    println!("🧬 Generating contigs...");
    match ParallelContigGenerator::generate_contigs_parallel(&cache_graph) {
        Ok(contigs) => {
            println!("✅ PIPELINE SUCCESS! Generated {} contigs from {} reads", contigs.len(), reads.len());
            
            if contigs.is_empty() {
                println!("🚨 CRITICAL BUG: Zero contigs generated despite {} nodes and {} edges", 
            ...
            for (i, contig) in contigs.iter().enumerate() {
                println!("  Contig {}: {}bp, coverage={:.1}x, sequence='{}'", 
                        i+1, contig.length, contig.coverage, contig.sequence);
            ...
            assert!(contigs.len() < reads.len(),
                   "Should produce fewer contigs ({}) than reads ({})", contigs.len(), reads.len());
        }
        Err(e) => {
