...
use meta_forge::core::data_structures::{
    AssemblyChunk, CanonicalKmer, CorrectedRead, CorrectionMetadata,
};
...
fn test_kmer_creation() {
    ...
    assert!(kmer.hash != 0);
    println!(
        "✅ K-mer creation works: hash={}, canonical={}",
        kmer.hash, kmer.is_canonical
    );
}
...
fn test_assembly_chunk_add_read() -> Result<()> {
    ...
    println!(
        "📊 Chunk stats: {} reads processed, {} nodes created",
        chunk.processing_stats.reads_processed, chunk.processing_stats.nodes_created
    );

    assert_eq!(chunk.processing_stats.reads_processed, 1);
    assert!(
        chunk.processing_stats.nodes_created > 0,
        "No nodes were created!"
    );
    assert!(
        !chunk.graph_fragment.nodes.is_empty(),
        "Graph fragment is empty!"
    );

    // Check that edges were created between consecutive k-mers
    assert!(
        chunk.processing_stats.edges_created > 0,
        "No edges were created!"
    );

    println!(
        "✅ AssemblyChunk.add_read created {} nodes and {} edges",
        chunk.processing_stats.nodes_created, chunk.processing_stats.edges_created
    );
...
fn test_overlapping_reads_create_connected_graph() -> Result<()> {
    ...
    println!(
        "📊 Connected graph: {} reads, {} nodes, {} edges",
...
fn test_parallel_contig_generator() -> Result<()> {
    ...
    println!(
        "📊 ParallelContigGenerator found {} contigs from {} nodes",
        contigs.len(),
        nodes.len()
    );
...
fn test_mini_assembly_pipeline() -> Result<()> {
    ...
    println!(
        "📊 Mini pipeline: {} reads → {} nodes → {} edges",
        chunk.processing_stats.reads_processed,
        chunk.processing_stats.nodes_created,
        chunk.processing_stats.edges_created
    );
        ...
        println!(
            ...
            if contig.sequence.len() > 20 {
                format!(
                    "{}...{}",
                    &contig.sequence[..10],
                    &contig.sequence[contig.sequence.len() - 10..]
                )
            } else {
    ...
    assert!(
        contigs.len() > 0,
        "❌ CRITICAL: No contigs generated in end-to-end test!"
    );
