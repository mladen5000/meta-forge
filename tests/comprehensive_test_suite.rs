...
impl TestDataGenerator {
    ...
    pub fn create_test_fastq(reads: &[CorrectedRead]) -> Result<NamedTempFile> {
        ...
        for read in reads {
            ...
            writeln!(file, "+")?;
            let quality_string: String = read
                .quality_scores
                .iter()
                .map(|&q| (q + 33) as char)
                .collect();
...
mod integration_tests {
    ...
    fn test_complete_pipeline_workflow() -> Result<()> {
        ...
        assert_eq!(
            ...
            feature_extractor.config.sequence_feature_dim
        );
    ...
    fn test_database_integration_workflow() -> Result<()> {
        ...
        assert_eq!(
            retrieved_name,
            Some("Escherichia coli str. K-12 substr. MG1655".to_string())
        );

        // Test sequence operations
        let test_sequences = vec![SequenceEntry {
            id: 0,
        ...
        assert_eq!(
            retrieved_features[0].features,
            training_examples[0].features
        );
    ...
    fn test_feature_extraction_consistency() -> Result<()> {
        ...
        // Should be identical
        assert_eq!(
            features1.sequence_features.len(),
            features2.sequence_features.len()
        );

        for (f1, f2) in features1
            .sequence_features
            .iter()
            .zip(features2.sequence_features.iter())
        {
        ...
        for (sequence, pattern_type) in patterned_sequences {
            ...
            assert_eq!(
                features.sequence_features.len(),
                extractor.config.sequence_feature_dim
            );
            assert!(
                features.metadata.complexity_score >= 0.0
                    && features.metadata.complexity_score <= 1.0
            );
            assert!(features.metadata.gc_content >= 0.0 && features.metadata.gc_content <= 1.0);

            println!(
                "  Extracted features for {}: complexity={:.3}, gc={:.3}",
    ...
    fn test_large_dataset_handling() -> Result<()> {
        ...
        println!(
            "✅ Large dataset handling test passed ({}ms)",
    ...
    fn test_edge_cases() -> Result<()> {
        ...
        let ambiguous_reads = vec![CorrectedRead {
            id: 0,
        ...
        assert_eq!(
            empty_features.sequence_features.len(),
            extractor.config.sequence_feature_dim
        );
        ...
        assert_eq!(
            short_features.sequence_features.len(),
            extractor.config.sequence_feature_dim
        );
        ...
        assert_eq!(
            mono_features.sequence_features.len(),
            extractor.config.sequence_feature_dim
        );
...
mod unit_tests {
    ...
    fn test_feature_extraction_components() {
        ...
        let comp_features = extractor
            .extract_composition_features(test_sequence)
            .unwrap();
        ...
        let complexity_features = extractor
            .extract_complexity_features(test_sequence)
            .unwrap();
...
mod benchmarks {
    ...
    pub fn benchmark_kmer_extraction(c: &mut Criterion) {
        ...
        c.bench_function("kmer_extraction_1kb", |b| {
            b.iter(|| black_box(extractor.extract_minimizers(black_box(&sequence)).unwrap()))
        });
    }
        ...
        for size in [100, 500, 1000, 5000].iter() {
            ...
            group.bench_with_input(BenchmarkId::new("sequence_length", size), size, |b, _| {
                b.iter(|| {
                    black_box(
                        extractor
                            .extract_sequence_features(black_box(&sequence))
                            .unwrap(),
                    )
                })
            });
    ...
    pub fn benchmark_database_operations(c: &mut Criterion) {
        ...
        c.bench_function("database_taxonomy_lookup", |b| {
            b.iter(|| black_box(db.get_taxonomy_name(black_box(511145)).unwrap()))
        });
        ...
        c.bench_function("database_sequence_insert", |b| {
            b.iter(|| black_box(db.insert_sequences(black_box(&sequences)).unwrap()))
        });
    }
