...
mod assembly_robustness_tests {
    ...
    fn create_test_read_with_metadata(
        id: usize,
        sequence: &str,
    ...
    fn test_assembly_with_ambiguous_nucleotides() {
        ...
        assert!(
            result.is_ok(),
            "Assembly should handle ambiguous nucleotides gracefully"
        );
    ...
    fn test_assembly_with_very_low_coverage() {
        ...
        assert_eq!(
            ...
            "Low coverage reads should be filtered"
        );
        assert_eq!(
            graph.graph_fragment.nodes.len(),
            0,
            "Low coverage nodes should be removed"
        );
    }
    ...
    fn test_assembly_with_very_short_reads() {
        ...
        assert_eq!(
            graph.graph_fragment.nodes.len(),
            0,
            "Short reads should produce no k-mers"
        );
    ...
    fn test_assembly_graph_statistics_accuracy() {
        ...
        assert!(
            stats.coverage_mean >= 0.0,
            "Coverage mean should be non-negative"
        );
        assert!(stats.n50 >= 0, "N50 should be non-negative");
        assert!(
            stats.largest_contig >= 0,
            "Largest contig should be non-negative"
        );

        if stats.num_contigs > 0 {
            assert!(
                stats.largest_contig <= stats.total_length,
                "Largest contig can't exceed total length"
            );
        }
    }
    ...
    fn test_bit_packed_kmer_canonical_consistency() {
        ...
        // Debug output
        println!(
            "Forward ATCG: packed={:?}, hash={}",
            forward_kmer.packed_data, forward_kmer.hash
        );
        println!(
            "Reverse CGAT: packed={:?}, hash={}",
            reverse_comp.packed_data, reverse_comp.hash
        );
    ...
    fn test_assembly_error_recovery() {
        ...
        assert!(
            result.is_ok(),
            "Assembly should recover from problematic reads"
        );
        ...
        assert!(
            ...
            "Should have nodes from good reads"
        );
    }
    ...
    fn test_memory_efficient_large_input() {
        ...
        assert!(
            !graph.graph_fragment.nodes.is_empty(),
            "Large input should produce nodes"
        );
        assert!(
            graph.assembly_stats.num_contigs > 0,
            "Large input should produce contigs"
        );
    }
