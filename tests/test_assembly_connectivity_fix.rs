...
fn test_kmer_edge_connectivity() -> Result<()> {
    ...
    assert_eq!(fragment.nodes.len(), expected_nodes, 
               "Should have {} nodes for k-mer size {}", expected_nodes, k);
    
    // CRITICAL TEST: Edges should connect consecutive k-mers
    assert!(fragment.edges.len() > 0, 
            "CRITICAL: No edges found - this indicates the 1:1:1 ratio bug!");
    
    assert_eq!(fragment.edges.len(), expected_edges,
               "Should have {} edges connecting consecutive k-mers", expected_edges);
    ...
    for edge in &fragment.edges {
        node_connections.entry(edge.from_hash).or_insert(vec![]).push(edge.to_hash);
    }
    
    let connected_nodes = node_connections.len();
    println!("  - Connected nodes: {} out of {}", connected_nodes, fragment.nodes.len());
    
    // Should have n-1 nodes with outgoing connections (last node has no outgoing edge)
    assert_eq!(connected_nodes, expected_edges,
...
fn test_multiple_reads_clustering() -> Result<()> {
    ...
    assert_ne!(fragment.nodes.len(), num_reads,
               "CRITICAL: Nodes equal reads ({}) - indicates 1:1:1 bug!", num_reads);
    
    // Should have fewer unique k-mers due to overlaps
    let total_possible_kmers: usize = reads.iter()
    ...
    assert!(fragment.nodes.len() < total_possible_kmers,
...
fn test_graph_fragment_validation() -> Result<()> {
    ...
    assert!(fragment.add_edge(valid_edge).is_ok(), "Valid edge should succeed");
    
    // Test invalid edge (missing node)
    let invalid_edge = GraphEdge::new(kmer1.hash, kmer3.hash, 1);
    assert!(fragment.add_edge(invalid_edge).is_err(), "Invalid edge should fail");
    
    // Test duplicate edge (should merge weights)
    let duplicate_edge = GraphEdge::new(kmer1.hash, kmer2.hash, 2);
    assert!(fragment.add_edge(duplicate_edge).is_ok(), "Duplicate edge should merge weights");
    
    // Verify edge weight was updated
    let edge = fragment.edges.iter().find(|e| e.from_hash == kmer1.hash && e.to_hash == kmer2.hash).unwrap();
