...
mod assembly_graph_builder_tests {
    ...
    fn test_assembly_graph_builder_with_overlapping_reads() {
        ...
        let high_coverage_nodes: Vec<_> = graph.graph_fragment.nodes
            .values()
            .filter(|node| node.coverage > 1)
            .collect();
        
        assert!(!high_coverage_nodes.is_empty(), "Should have some high coverage nodes from overlaps");
    }
...
mod assembly_chunk_tests {
    ...
    fn test_assembly_chunk_finalization() {
        ...
        for node in chunk.graph_fragment.nodes.values() {
            assert!(matches!(node.node_type, 
                NodeType::LowCoverage | NodeType::Unique | 
                NodeType::HighCoverage | NodeType::Repetitive));
        }
    }
    ...
    fn test_assembly_chunk_coverage_accumulation() {
        ...
        assert!(!high_coverage_nodes.is_empty(), "Should have high coverage nodes from overlapping reads");
    }
}
