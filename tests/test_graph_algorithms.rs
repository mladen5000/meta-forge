...
mod graph_construction_tests {
    ...
    fn test_complex_graph_with_multiple_components() {
        ...
        let comp1_neighbors: HashSet<u64> = comp1_nodes.iter()
...
mod bubble_detection_tests {
    ...
    fn test_complex_bubble_with_multiple_paths() {
        ...
        let sequences = vec!["AAAA", "TTTT", "GGGG", "CCCC", "ATGC", "CGTA", "TACG", "GCTA"];
        let mut nodes = Vec::new();
    ...
    fn test_branching_without_reconvergence() {
        ...
        let bubbles = fragment.find_bubbles();
        assert!(bubbles.is_empty(), "Branching without reconvergence should have no bubbles");
    }
}
...
mod path_reconstruction_tests {
    ...
    fn test_empty_path_reconstruction() {
        ...
        let result = fragment.reconstruct_sequence_from_path(&empty_path).unwrap();
...
mod graph_statistics_tests {
    ...
    fn test_coverage_statistics_update() {
        ...
        // Check distribution
        assert_eq!(fragment.coverage_stats.coverage_distribution.get(&5), Some(&1));
        assert_eq!(fragment.coverage_stats.coverage_distribution.get(&10), Some(&1));
        assert_eq!(fragment.coverage_stats.coverage_distribution.get(&15), Some(&1));
        assert_eq!(fragment.coverage_stats.coverage_distribution.get(&20), Some(&1));
    }
