...
mod end_to_end_pipeline_tests {
    ...
    fn test_complete_pipeline_small_dataset() {
        ...
        assert!(
            !assembly.graph_fragment.nodes.is_empty(),
            "Should have graph nodes"
        );
        assert!(
            !assembly.graph_fragment.edges.is_empty(),
            "Should have graph edges"
        );
        assert!(
            !assembly.contigs.is_empty(),
            "Should have generated contigs"
        );
    ...
    fn test_pipeline_with_varying_quality_reads() {
        ...
        let high_coverage_nodes: Vec<_> = assembly
            .graph_fragment
            .nodes
            .values()
        ...
        assert!(
            ...
            "Should have overlapping coverage from similar sequences"
        );
    }
    ...
    fn test_pipeline_error_handling_invalid_reads() {
        ...
        assert!(
            result.is_ok(),
            "Pipeline should handle problematic reads gracefully"
        );
    ...
    fn test_large_scale_integration() {
        ...
        // Verify scalability
        assert!(
            assembly.graph_fragment.nodes.len() > 10,
            "Should have substantial graph"
        );
        assert!(
            assembly.assembly_stats.total_length > 100,
            "Should have reasonable assembly size"
        );
...
mod field_access_validation_tests {
    ...
    fn test_assembly_graph_builder_constructor() {
        ...
        let reads = vec![end_to_end_pipeline_tests::create_realistic_read(
            0, "ATCGATCG", 35,
        )];
        let result = builder.build(&reads);
        assert!(
            result.is_ok(),
            "Build method should work with fixed constructor"
        );
    }
    ...
    fn test_assembly_chunk_method_calls() {
        ...
        chunk.finalize();
        assert!(
            chunk.processing_stats.memory_usage_bytes > 0,
            "finalize should calculate memory usage"
        );
    }
...
mod error_handling_integration_tests {
    ...
    fn test_extreme_coverage_handling() {
        ...
        for i in 0..100 {
            reads.push(end_to_end_pipeline_tests::create_realistic_read(
                i,
                base_sequence,
                35,
            ));
        ...
        assert!(
            max_coverage >= 50,
            "Should have high coverage from many identical reads"
        );
        ...
        assert!(
            repetitive_nodes > 0,
            "Should classify high-coverage nodes as repetitive"
        );
    }
