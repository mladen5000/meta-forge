...
mod abundance_estimation_tests {
    ...
    fn test_coverage_calculation_accuracy() {
        ...
        let expected_mean = test_cases.iter().map(|(_, c)| *c as f64).sum::<f64>() / test_cases.len() as f64;
    ...
    fn test_coverage_distribution_accuracy() {
        ...
        for (&expected_coverage, &expected_count) in &coverage_counts {
            let actual_count = fragment.coverage_stats.coverage_distribution
                .get(&expected_coverage)
                .copied()
                .unwrap_or(0);
            assert_eq!(actual_count, expected_count,
                      "Coverage {} should have {} nodes", expected_coverage, expected_count);
        }
    ...
    fn test_path_coverage_estimation() {
        ...
        let expected_avg = path_data.iter().map(|(_, c)| *c as f64).sum::<f64>() / path_data.len() as f64;
    ...
    fn test_coverage_with_overlapping_reads() {
        ...
        let overlapping_reads = vec![
            ...
            create_test_read_with_quality(1, &base_sequence[1..], vec![35; base_sequence.len() - 1]), // Offset by 1
            create_test_read_with_quality(2, &base_sequence[2..], vec![25; base_sequence.len() - 2]), // Offset by 2
        ];
        ...
        let coverages: Vec<u32> = chunk.graph_fragment.nodes.values().map(|n| n.coverage).collect();
        ...
        assert!(max_coverage > min_coverage, "Overlapping reads should create coverage variation");
        assert!(*max_coverage > 1, "Some k-mers should have coverage > 1 from overlaps");
    }
    ...
    fn test_abundance_estimation_with_quality_scores() {
        ...
        let high_coverage_nodes: Vec<_> = chunk.graph_fragment.nodes
            .values()
...
mod complexity_scoring_tests {
    ...
    fn test_sequence_complexity_extremes() {
        ...
        assert!(min_complexity < 0.1, "Homopolymer should have very low complexity");
        
        // Test maximum complexity (perfectly balanced)
        let max_complexity = calculate_sequence_complexity("ATCGATCGATCG");
        assert!(max_complexity > 0.9, "Balanced sequence should have high complexity");
    ...
    fn test_gc_content_calculation_accuracy() {
        ...
        for (sequence, expected) in test_cases {
            ...
            assert!((actual - expected).abs() < 1e-10, 
                   "GC content of '{}' should be {}, got {}", sequence, expected, actual);
        }
    }
